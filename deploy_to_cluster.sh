...
# Update the service to use the new taskDefinition
aws ecs update-service --service $SVC_ARN --task-definition $NEW_TD_ARN

# Scale the service back up to 1
aws ecs update-service --service $SVC_ARN --desired-count 1
